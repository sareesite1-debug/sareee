      ...
      <div className="bg-ivory min-h-screen">
        <section ref={heroRef} className="relative bg-emerald-deep text-ivory pt-40 pb-28 overflow-hidden perspective-1000">
          ...
          <motion.div style={{ y, opacity }} className="container mx-auto px-6 lg:px-10 relative z-10 text-center max-w-3xl">
            <motion.div initial={{ opacity: 1, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="flex justify-center items-center gap-3 mb-6">
            ...
            <motion.p initial={{ opacity: 1 }} animate={{ opacity: 1 }} transition={{ delay: 0.7, duration: 0.8 }} className="text-ivory/60 text-lg font-body font-light">Explore our curated collections, each a testament to India's varied weaving heritage.</motion.p>
          </motion.div>
        </section>
        ...
        <section className="container mx-auto px-6 lg:px-10 py-24">
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
              {categories.map((c, i) => (
                <motion.div key={c.id} initial={{ opacity: 1, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ delay: i * 0.1, duration: 0.9, type: "spring", stiffness: 100 }}>
                    ...
                    <Link to={`/categories/${c.slug}`} className="group block h-full bg-ivory-deep border border-gold/10 p-4">
                      <div className="relative overflow-hidden aspect-[4/3] mb-6 img-fit">
                        ...
                        <motion.div className="absolute top-4 right-4 bg-ivory/90 backdrop-blur-md px-3 py-1 border border-gold/20"
                          initial={{ x: 20, opacity: 1 }} whileHover={{ x: 0, opacity: 1 }} transition={{ type: "spring", stiffness: 300, damping: 25 }}>
    ...
    <div className="bg-ivory min-h-screen">
      <section className="relative pt-40 pb-20 overflow-hidden bg-ivory-deep border-b border-gold/15">
        ...
        <div className="container mx-auto px-6 lg:px-10 relative z-10">
          <motion.div initial={{ opacity: 1, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
          ...
          {loading ? (
            ...
            <div className="max-w-3xl">
              <motion.div initial={{ opacity: 1, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="flex items-center gap-3 mb-5">
              ...
              {activeCategory.description && (
                <motion.p initial={{ opacity: 1, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.8 }} className="text-ink-soft text-lg font-body font-light leading-relaxed">
                  {activeCategory.description}
                </motion.p>
              )}
            </div>
      ...
      <section className="container mx-auto px-6 lg:px-10 py-20">
        ...
        {loading ? (
          ...
          <motion.div initial={{ opacity: 1, scale: 0.9 }} animate={{ opacity: 1, scale: 1 }} className="border border-dashed border-gold/25 py-32 text-center glass-card">
